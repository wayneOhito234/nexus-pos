...
//   27, 112 = ESC p
//   0       = pin 2 (use 1 for pin 5 on drawers wired that way)
//   25, 250 = on and off pulse timings
const KICK_PIN_2 = Buffer.from([27, 112, 0, 25, 250]);
const KICK_PIN_5 = Buffer.from([27, 112, 1, 25, 250]);

// The drawer connects to the printer's DK port, not to the PC, so opening it
// means sending raw bytes to the printer. Copying a file to the printer's
...
function openDrawer({ shareName = 'POS80C', pin = 2 } = {}) {
  return new Promise((resolve, reject) => {
    ...
    exec(`copy /b "${tmpFile}" \\\\localhost\\${shareName}`, { shell: 'cmd.exe' }, (err, stdout, stderr) => {
      fs.unlink(tmpFile, () => {});

      if (err) {
        return reject(new Error(
          `Drawer did not open. Check the printer is shared as "${shareName}". ${stderr || err.message}`
        ));
      }
      resolve({ ok: true });
    });
